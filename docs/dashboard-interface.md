...
- System: all global alarms are shown here only, as compact rows with severity and full message text. Other pages retain their device status and local configuration feedback, without repeated alarm banners. NAS and technical details are expandable.
...
The original gradient headers, rounded cards, subtle shadows and pill-shaped controls are retained alongside the revised layout and navigation. Normal pages grow with their content. Only the optional detailed timeline scrolls horizontally. Focus indicators, touch-sized controls and reduced-motion support are shared across pages. The interface itself remains Dutch; this documentation is bilingual.
...
De eerdere uitstraling met kleurverlopen, ronde kaarten, subtiele schaduwen en pilvormige knoppen blijft behouden. Algemene alarmen staan uitsluitend op **Systeem**, in compacte regels met de volledige melding. Apparaatstatus en lokale configuratiecontrole blijven wel bij de betreffende bediening staan.
