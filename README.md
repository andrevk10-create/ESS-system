...
### Predictive home climate control (commissioning required)

The Climate page now includes a bounded controller for two explicitly mapped Tado rooms and domestic hot water. Room presets start at **18–22 °C, normally 21 °C**. By default it follows the current native Tado schedule; optional custom comfort times are available. Zones remain disabled until configured and commissioned.

It compares complete heating blocks against future electricity prices, considers sustained PV export, battery reserve and hourly weather, and checks fresh per-phase grid readings plus the dedicated heat-pump meter before increasing demand. The electrical setting is a software start condition, **not a hard power limiter** and not the heat pump's thermal rating.

Actual control uses 30-minute `tado.set_climate_timer` overrides and bounded `water_heater.set_temperature` requests. Commands are saved before dispatch and checked again against live readings. Missing target feedback blocks further optimization instead of endlessly retrying. Manual overrides are respected; no heat-pump mains switch is used. Climate settings and command ownership survive pulls/restarts in the private charging-preferences backup.

Hot-water buffering additionally requires a verified, fresh `binary_sensor.dhw_hygiene_active` mapping. Without it, native hot-water control stays in charge. Do not invent an always-off guard. Cloud loss may leave an already applied hot-water target in place until connectivity returns. Commission target readback and timed restoration before unattended use.

This is an initial rule-based controller, not a learned building model or a fully optimized shared EV/battery/heat-pump planner. No measured savings or live commissioning are implied. Decision telemetry is published as `sensor.ess_climate_control`; include it in HA Recorder if historical analysis is required. See the [detailed design and implementation boundaries](docs/climate-control-design.md) (Dutch).

### Audi climate quick-start compatibility

Audi Connect API-level-1 climate start is model dependent. Some vehicles reject the request body generated by Audi Connect 2.3.1 and only accept a plain start request without temperature, seat, window, or mode options. The dashboard deliberately sends only the Home Assistant `device_id`. If Home Assistant still logs `Unable to start climate control of vehicle`, first verify and then apply the included narrowly scoped compatibility fix from the repository directory:
...
Restart Home Assistant afterwards. The script creates `audi_services.py.before-smart-ess-climate-fix`, refuses unknown source layouts, and changes only the API-level-1 climate-start branch. A HACS update may replace the patched integration; rerun `--check` after updating Audi Connect. This workaround is based on the independently reproduced [Audi Connect issue #771](https://github.com/audiconnect/audi_connect_ha/issues/771) and should be tested under supervision because Audi can change its private cloud API without notice.
